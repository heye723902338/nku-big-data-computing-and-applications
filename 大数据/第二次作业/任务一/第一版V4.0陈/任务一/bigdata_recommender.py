...
def evaluate_model_on_validation(model_name: str, ratings: Sequence[Rating], args: argparse.Namespace) -> float:
    train_part, valid_part = make_validation_split(
        ratings,
        valid_ratio=args.valid_ratio,
        seed=args.seed,
    ...
    t0 = time.perf_counter()
    model = fit_selected_model(model_name, train_part, args, verbose=False)
    elapsed = time.perf_counter() - t0
    score = rmse(model, valid_part)
    print(f"  {model_name:8s} RMSE={score:.6f}, train_time={elapsed:.2f}s")
    return score
...
def get_peak_memory_mb() -> float | None:
    ...
    try:
        ...
        peak_bytes = max(counters.PeakWorkingSetSize, counters.PeakPagefileUsage, counters.PrivateUsage)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.model == "auto":
        ...
        evaluate_model_on_validation(selected_model_name, ratings, args)
